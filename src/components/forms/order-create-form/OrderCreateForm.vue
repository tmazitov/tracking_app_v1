<template>
	<transition name="modal">
		<div class="order_create_form" v-bind:style="{
			...
			<CreateOrderBillModal v-if="data.priceList" :isOpen="data.billIsOpen" :closer="closeBill" :form="form.values"
				:submit="orderSubmitHandler" :priceList="data.priceList" />
			...
			<div class="form">
				...
				<div class="form__tabs-content">
					<div class="form__page-1" v-if="data.selectedTab == 0">
						<div class="form__datetime-title">
							...
							<div class="form__datetime">
								<div class="datetime__date">
									<ion-input v-model="fields.date.value" type="date" name="date" id="date"
										:color="form.errors.value['title'] ? 'danger' : 'primary'" label="Дата"
										label-placement="floating" fill="solid">
									</ion-input>

									<ion-input type="time" v-model="fields.start.value" name="start" @ionInput="onTimeInput"
										ref="inputTimeRef" :color="form.errors.value['title'] ? 'danger' : 'primary'"
								...
								<RNumberInput inputType="hour" label-placement="floating" fill="solid" label="Часы работы"
									v-model:value="fields.duration.value" :step="1" :min="2" :max="1000" />
							</div>
							...
							<div class="form__title-container">
								<ion-input type="text" v-model="fields.title.value" name="title"
									:color="form.errors.value['title'] ? 'danger' : 'primary'" label="Название заказа"
									fill="solid" label-placement="floating" placeholder="По умолчанию адрес первой точки">
								</ion-input>
							</div>
						...
						<div class="form__comment-container">
							<ion-textarea name="comment" label="Комментарий заказа" labelPlacement="floating" fill="solid"
								v-model="fields.comment.value">
							</ion-textarea>
						</div>

						<RSelector :label="'Тип заказа'" v-model:current-item="fields.currentOrderType.value"
							:items="orderTypes" :selector="selectOrderType" :multiple="true" />
				...
				<div class="form__submit-container" v-if="data.selectedTab == 0">
					<ion-text v-if="form.errors.value.points && data.submitIsTouched" color="danger">
...
import { makeErrorToast, makeSuccessToast } from "@/assets/toast";
...
export default {
	...
	setup(props) {
		...
		const orderSubmitHandler = () => {
			...
			let toastPromise:Promise<HTMLIonToastElement>
			let promise
			...
			if (orderToUpdate) {
				...
				.then(response => {
					...
					updateOrder.value.update(response.data)
					toastPromise = makeSuccessToast('Заказ обновлен успешно!')
				})
				.catch(err => {
					toastPromise = makeErrorToast('Ошибка при обновлении заказа!')
				})
			} else {
				...
				.then(response => {
					...
					toastPromise = makeSuccessToast('Заказ создан успешно!')
				})
				.catch(err => {
					toastPromise = makeErrorToast('Ошибка при создании заказа!')
				})
			}
			promise.then(() => {
